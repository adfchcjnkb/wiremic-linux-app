...
NavRailButton::NavRailButton(const QString& iconPath, const QString& label,
                              QWidget* parent)
...
void NavRailButton::setSelectedProgress(qreal progress) {
  selectedProgress_ = progress;
  update();
}
...
void NavRailButton::enterEvent(QEnterEvent* event) {
  hovered_ = true;
  update();
  QWidget::enterEvent(event);
}

void NavRailButton::leaveEvent(QEvent* event) {
  hovered_ = false;
  update();
  QWidget::leaveEvent(event);
}

void NavRailButton::paintEvent(QPaintEvent* /*event*/) {
  ...
  if (selectedProgress_ > 0.001) {
    ...
    painter.fillPath(path, theme::kGlassFill);
  }
  ...
  painter.drawPixmap(QRectF(pillRect.left() + 14,
                             pillRect.center().y() - iconSize / 2.0, iconSize,
                             iconSize),
                      icon, icon.rect());
