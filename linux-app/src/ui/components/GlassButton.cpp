...
void GlassButton::setBusy(bool busy) {
  busy_ = busy;
  setEnabled(!busy);
  if (busy_ && spinTimerId_ < 0) {
    spinTimerId_ = startTimer(16);
  } else if (!busy_ && spinTimerId_ >= 0) {
    killTimer(spinTimerId_);
    spinTimerId_ = -1;
  }
  update();
}
...
void GlassButton::setHoverProgress(qreal progress) {
...
void GlassButton::timerEvent(QTimerEvent* event) {
  if (event->timerId() == spinTimerId_) {
    spinAngle_ = (spinAngle_ + 8) % 360;
    update();
    return;
  }
...
void GlassButton::paintEvent(QPaintEvent* /*event*/) {
  ...
  if (variant_ == Variant::Primary) {
    ...
    if (hoverProgress_ > 0.0) {
      painter.setOpacity(hoverProgress_ * 0.12);
      painter.fillPath(path, Qt::white);
      painter.setOpacity(1.0);
    }
  } else {
    const QColor base = variant_ == Variant::Danger
                             ? QColor(255, 93, 120, 20)
                             : theme::kGlassFill;
    const QColor hoverFill = variant_ == Variant::Danger
                                  ? QColor(255, 93, 120, 34)
                                  : theme::kGlassFillHover;
  ...
  if (busy_) {
    ...
    QRectF spinnerRect(bounds.center().x() - diameter / 2.0,
                        bounds.center().y() - diameter / 2.0, diameter,
                        diameter);
  ...
  const QColor textColor =
      variant_ == Variant::Primary
          ? Qt::white
          : (variant_ == Variant::Danger ? theme::kDanger : theme::kTextPrimary);
