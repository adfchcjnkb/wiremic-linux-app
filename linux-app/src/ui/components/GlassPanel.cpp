...
GlassPanel::GlassPanel(QWidget* parent)
    : QWidget(parent),
      fillColor_(theme::kCardFill),
      borderColor_(theme::kGlassBorder) {
  setElevated(true);
}
...
void GlassPanel::setFillColor(const QColor& color) {
  fillColor_ = color;
  update();
}
...
void GlassPanel::paintEvent(QPaintEvent* /*event*/) {
  ...
  QPainterPath path;
  path.addRoundedRect(rect().adjusted(0, 0, -1, -1), cornerRadius_,
                       cornerRadius_);
  ...
  topHalf.addRoundedRect(
      QRectF(0, 0, width(), height() * 0.5 + cornerRadius_), cornerRadius_,
      cornerRadius_);
